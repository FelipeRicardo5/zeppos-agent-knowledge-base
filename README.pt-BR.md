...
| `parse` — sete frentes: páginas de referência, os runtimes do celular, `static/llms`, imports dos samples, apps de exemplo, guias, lista de dispositivos | implementado |
...
- **30 de 381 símbolos não têm descrição.** 24 são `OBSERVED` — vistos só em código de sample, que não carrega prosa. Os outros 6 são páginas que genuinamente não têm nada entre título e primeira seção (`@zos/crypto`, dois widgets de `@zos/ui`).
...
2. **parse** — sete frentes independentes sobre o cache bruto:
